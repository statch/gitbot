...
class Repo(commands.Cog):
    ...
    async def repo_command_group(self,
    ...
    async def repo_info_command(self, ctx: commands.Context,
                                repo: str) -> None:
        if hasattr(ctx, "data"):
        ...
        embed = discord.Embed(
            ...
            if r["primaryLanguage"] else 0xEFEFEF,
            title=f"{repo}",
            description=None,
            url=r["url"],
        )
        ...
        if r["description"] is not None and len(r["description"]) != 0:
            embed.add_field(
                ...
                value=
                f"```{re.sub(MD_EMOJI_RE, '', r['description']).strip()}```",
            )

        watchers: str = (f"Has [{watch} watchers]({r['url']}/watchers)"
                         if watch != 1 else
                         f"Has [one watcher]({r['url']}/watchers) ")
        if watch == 0:
            watchers: str = f"Has no watchers"
        stargazers: str = (f"no stargazers\n" if star == 0 else
                           f"[{star} stargazers]({r['url']}/stargazers)\n")
        if star == 1:
        ...
        issues: str = (f'Doesn\'t have any [open issues]({r["url"]}/issues)\n'
                       ...
                       f"Has [{open_issues} open issues]({r['url']}/issues)\n")
        if open_issues == 1:
            issues: str = f"Has only one [open issue]({r['url']}/issues)\n"

        forks: str = (
            f"No one has forked this repo, yet\n" if r["forkCount"] == 0 else
            f"Has been forked [{r['forkCount']} times]({r['url']}/network/members)\n"
        )
        if r["forkCount"] == 1:
        ...
        homepage: tuple = (
            r["homepageUrl"]
            if "homepageUrl" in r and r["homepageUrl"] else None,
            "Homepage",
        )
        links: list = [homepage]
        link_strings: list = []
        ...
        if ("licenseInfo" in r and r["licenseInfo"] is not None
    ...
    @repo_command_group.command(
        name="--files",
        aliases=["-f", "files", "-files", "-s", "-src", "-fs", "fs"])
    async def repo_files_command(self, ctx: commands.Context,
                                 repo_or_path: str) -> None:
        is_tree: bool = False
        ...
        if not src:
            if is_tree:
                ...
                await ctx.send(
                    f"{self.emoji} This repository **doesn't exist!**")
            return
        files: list = [
            f"{self.f}  [{f['name']}]({f['html_url']})" if f["type"] == "file"
            else f"{self.fd}  [{f['name']}]({f['html_url']})" for f in src[:15]
        ]
        if is_tree:
        ...
        embed = discord.Embed(
            color=0xEFEFEF,
            title=f"{repo_or_path}" if len(repo_or_path) <= 60 else "/".join(
                repo_or_path.split("/", 2)[:2]),
            description="\n".join(files),
            url=link,
        )
        if len(src) > 15:
    ...
    @repo_command_group.command(name="--download",
                                aliases=["-download", "download", "-dl"])
    ...
    async def download_command(self, ctx: commands.Context, repo: str) -> None:
        ...
        if src_bytes is None:  # pylint: disable=no-else-return
            return await msg.edit(
                content=f"{self.e}  This repo **doesn't exist!**")

...
class Org(commands.Cog):
    ...
    async def org_command_group(self,
    ...
    async def org_info_command(self, ctx: commands.Context,
        ...
        if not org:
            if hasattr(ctx, "invoked_with_stored"):
                ...
                await ctx.send(
                    f"{self.emoji} This organization **doesn't exist!**")
            return None
        ...
        repos: str = (
            "Has no repositories, yet\n" if org["public_repos"] == 0 else
            f"Has a total of [{org['public_repos']} repositories]({org['html_url']})\n"
        )
        ...
        twitter: tuple = (
            ...
            if "twitter_username" in org
            and org["twitter_username"] is not None else None,
            "Twitter",
        )

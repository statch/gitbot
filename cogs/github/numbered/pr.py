...
class PullRequest(commands.Cog):
    ...
    async def pull_request_command(
        self, ctx: commands.Context, repo: str, pr_number: Optional[str] = None
    ):
        if hasattr(ctx, "data"):
        ...
        assignee_strings = (
            assignee_strings
            if len(assignee_strings) <= 3
            else assignee_strings[:3] + f"- and {len(assignee_strings) - 3} more"
        )

        reviewer_strings = (
            reviewer_strings
            if len(reviewer_strings) <= 3
            else reviewer_strings[:3] + f"- and {len(reviewer_strings) - 3} more"
        )

        participant_strings = (
            participant_strings
            if len(participant_strings) <= 3
            else participant_strings[:3] + f"- and {len(participant_strings)} more"
        )

        cross_repo: str = (
            f"This pull request came from a fork." if pr["isCrossRepository"] else ""
        )
        ...
        embed.add_field(
            ...
            if participant_strings
            else f"No participants",
            inline=True,
        )
        embed.add_field(
            name="Assignees:",

...
class Gist(commands.Cog):
    ...
    async def gist_command(self,
        ...
        if (gists := len(data["gists"]["nodes"])) < 2:
            if gists == 0:
                ...
                await ctx.send(embed=await self.build_gist_embed(
                    data,
                    1,
                    footer=
                    "You didn't see the gist list because this user has only one gist.",
                ))
            return
        ...
        def gist_url(gist: dict) -> str:
            if not gist["description"]:
                return gist["url"]
            desc = (gist["description"] if len(gist["description"]) < 70 else
                    gist["description"][:67] + "...")
        ...
        while True:
            try:
                msg: discord.Message = await self.bot.wait_for(
                    "message",
                    check=lambda m: (m.channel.id == ctx.channel.id and m.
    ...
    async def build_gist_embed(self,
        ...
        stargazers = (
            ...
            if gist["stargazerCount"] != 1 else
            f"Has [one stargazer]({gist['url']}/stargazers)")
        if gist["stargazerCount"] == 0:
            stargazers = "Has no stargazers"
        comment_count = gist["comments"]["totalCount"]
        comments = (f"and [{comment_count} comments]({gist['url']})"
                    if comment_count != 1 else
                    f"and [one comment]({gist['url']})")
        if gist["stargazerCount"] == 0:
        ...
        embed.add_field(
            ...
            value=
            f"```{self.extension(first_file['extension'])}\n{first_file['text'][:449]}```",
        )

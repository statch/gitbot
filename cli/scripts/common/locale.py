...
def get_master_locale() -> OrderedDict:
    ...
    with open(os.path.join(LOCALE_DIR, f'{locale_index["master"]}.json'), 'r', encoding='utf8') as locale_file:
...
def save_changes(updated: OrderedDict, old: OrderedDict) -> None:
    if updated != old:
        with open(os.path.join(LOCALE_DIR, f'{old["meta"]["name"]}.last.json'), 'w+', encoding='utf8') as old_locale_file:
            old_locale_file.write(json.dumps(old, indent=2))
        with open(os.path.join(LOCALE_DIR, f'{updated["meta"]["name"]}.json'), 'w+', encoding='utf8') as locale_file:

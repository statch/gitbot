...
class User(commands.Cog):
    ...
    async def user_command_group(self,
    ...
    async def user_info_command(self, ctx: commands.Context,
                                user: str) -> None:
        if hasattr(ctx, "data"):
        ...
        occupation: str = (f'Works at {u["company"]}\n'
                           if "company" in u and u["company"] is not None else
                           "Isn't part of a company\n")
        orgs: str = (f"Belongs to {orgs_c} organizations\n" if orgs_c != 0 else
                     "Doesn't belong to any organizations\n")
        if orgs_c == 1:
            orgs: str = "Belongs to 1 organization\n"
        followers: str = (
            "Isn't followed by anyone" if u["followers"] == 0 else
            f"Has [{u['followers']} followers]({u['url']}?tab=followers)")

        if u["followers"] == 1:
            followers: str = f"Has only [1 follower]({u['url']}?tab=followers)"
        following: str = (
            "doesn't follow anyone, yet" if u["following"] == 0 else
            f"follows [{u['following']} users]({u['url']}?tab=following)")
        if u["following"] == 1:
            following: str = f"follows only [1 person]({u['url']}?tab=following)"
        follow: str = followers + " and " + following

        repos: str = (
            "Has no repositories, yet\n" if u["public_repos"] == 0 else
            f"Has a total of [{u['public_repos']} repositories]({u['url']}?tab=repositories)\n"
        )
        if u["public_repos"] == 1:
        ...
        twitter: tuple = (
            ...
            if "twitterUsername" in u else None,
            "Twitter",
        )
    ...
    async def user_repos_command(self, ctx: commands.Context,
                                 user: str) -> None:
        ...
        if not repos:
            await ctx.send(
                f"{self.emoji} This user doesn't have any **public repos!**")
            return

...
def dir_line_count(directory) -> int:
    return sum(
        map(lambda item: item_line_count(join(directory, item)),
            os.listdir(directory)))


LINES_OF_CODE: int = sum([
    dir_line_count("./cogs"),
    dir_line_count("./ext"),
    dir_line_count("./core")
])
...
class BotInfo(commands.Cog):
    ...
    @commands.command(name="uptime",
                      aliases=["--uptime", "-uptime", "up", "--up", "-up"])
    ...
    async def uptime_command(self, ctx: commands.Context) -> None:
        ...
        uptime_stamp: str = time_format.format(d=days,
                                               h=hours,
                                               m=minutes,
                                               s=seconds)
        embed: discord.Embed = discord.Embed(
            ...
            title=None,
            description=
            f"{self.s}  **I have been online for:**\n{uptime_stamp}",
        )
    ...
    @commands.command(
        name="privacy",
        aliases=["policy", "--privacy", "-privacy", "-policy", "--policy"],
    )
    @commands.cooldown(15, 30, commands.BucketType.member)
    async def privacy_policy(self, ctx: commands.Context) -> None:
        embed: discord.Embed = discord.Embed(
            color=0xEFEFEF, title=f"{self.emoji}  Privacy Policy")
        embed.add_field(
            name="What is stored?",
            inline=False,
            value=
            "The only data stored are your User ID and quick access users, repos and orgs.",
        )
        embed.add_field(
            name="How is all this used anyway?",
            inline=False,
            value=
            "It's used to provide storage for your saved users, repos and organizations, your User "
            "ID is essential for the Bot to know, what data is yours.",
        )
        embed.add_field(
            name="Who has access to this data?",
            inline=False,
            value=
            "Only the Bot's developer has access, no one else. Your data isn't viewed or accessed "
            "in any way unless it's required to provide the service.",
        )
        embed.add_field(
            name="How can I get rid of this stored data?",
            inline=False,
            value=
            "You can do that very easily by using the `git config -delete` command!",
        )
        embed.add_field(
            name="Who wrote this and can this be changed?",
            inline=False,
            value=
            "This privacy policy was written by [wulf](https://dsc.bio/wulf), the Bot's developer, "
    ...
    async def invite_command(self, ctx: commands.Context) -> None:
        embed: discord.Embed = discord.Embed(
            color=0xEFEFEF,
            description=
            f"[**Invite {self.bot.user.name}**](https://discord.com/oauth2/authorize?client_id"
    ...
    async def vote_command(self, ctx: commands.Context) -> None:
        embed: discord.Embed = discord.Embed(
            color=0xEFEFEF,
            description=
            "[**top.gg**](https://top.gg/bot/761269120691470357/vote) | [**botsfordiscord.com**]("
            "https://botsfordiscord.com/bot/761269120691470357) ",
        )
        embed.set_author(name=f"Vote for {self.bot.user.name}!",
    ...
    async def stats_command(self, ctx: commands.Context) -> None:
        ...
        memory: str = "**{:.3f}GB** of RAM".format(process.memory_info()[0] /
                                                   2.0**30)
        cpu: str = f"**{psutil.cpu_percent()}%** CPU, and"
        embed.add_field(
            name=f"{self.s_emoji}  Bot Stats",
            value=f"General stats regarding the Bot's functioning.",
            inline=False,
        )

...
class GitHubAPI:
    ...
    async def ghprofile_stats(self, name: str) -> Union[GhStats, None]:
        ...
        res = await (
            await self.ses.get(
                f"https://api.ghprofile.me/historic/view?username={name}"
            )
        ).json()
    ...
    async def get_user_repos(self, user: str) -> Optional[list]:
        try:
            return list(
                [
                    x
                    for x in await self.gh.getitem(f"/users/{user}/repos")
    ...
    async def get_org_repos(self, org: str) -> Union[List[dict], list]:
        try:
            res = list(
                [
                    x
                    for x in await self.gh.getitem(f"/orgs/{org}/repos")
                    if x["private"] is False
                ]
            )
            return res
    ...
    async def get_latest_release(self, repo: str) -> Optional[dict]:
        ...
        """.format(
            owner=owner, name=name, q=self._queries.release
        )

        data: dict = await self.post_gql(query, "repository")
        if data:
            data["release"] = (
                data["releases"]["nodes"][0] if data["releases"]["nodes"] else None
            )
            data["color"] = (
                int(data["primaryLanguage"]["color"][1:], 16)
    ...
    async def get_repo(self, repo: str) -> Optional[dict]:
        ...
        {{
          repository(name: "{name}", owner: "{owner}") {q}}}
        """.format(
            owner=owner, name=repository, q=self._queries.repo
        )
        ...
        if data:
            ...
            data["graphic"] = (
                data["openGraphImageUrl"] if data["usesCustomOpenGraphImage"] else None
            )
            data["release"] = (
                data["releases"]["nodes"][0]["tagName"]
                if data["releases"]["nodes"]
                else None
            )
        return data
        ...
        if isinstance(data, dict):
            ...
            data["reviewers"]["users"] = [
                (
                    ...
                    if "login" in o["node"]["requestedReviewer"]
                    else o["node"]["requestedReviewer"]["name"],
    ...
    async def get_last_pull_requests_by_state(
        self, repo: str, last: int = 10, state: str = "[OPEN]"
    ) -> Optional[List[dict]]:
    ...
    async def get_last_issues_by_state(
        ...
        """.format(
            name=repository, owner=owner, state=state, last=last, q=self._queries.issue
        )
    ...
    async def get_user(self, user: str):
        ...
        """.format(
            user=user, from_=YEAR_START, to=to
        )

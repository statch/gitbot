...
class PullRequest(commands.Cog):
    ...
    @commands.command(
        name="pr", aliases=["pull", "-pr", "--pr", "--pullrequest", "-pull"])
    ...
    async def pull_request_command(self,
        ...
        if hasattr(ctx, "data"):
            ...
            try:
                ...
                await ctx.send(
                    f"{self.e}  The second argument must be a pull request **number!**"
                )
                return
        ...
        files_changed: str = (
            f'[{pr["changedFiles"]} files]({pr["url"]}/files) '
            f"have been changed in {commits}\n")
        if pr["changedFiles"] == 1:
            files_changed: str = (
                f'[One file]({pr["url"]}/files) was changed '
        ...
        assignee_strings = (assignee_strings if len(assignee_strings) <= 3 else
                            assignee_strings[:3] +
                            f"- and {len(assignee_strings) - 3} more")

        reviewer_strings = (reviewer_strings if len(reviewer_strings) <= 3 else
                            reviewer_strings[:3] +
                            f"- and {len(reviewer_strings) - 3} more")

        participant_strings = (participant_strings
                               if len(participant_strings) <= 3 else
                               participant_strings[:3] +
                               f"- and {len(participant_strings)} more")

        cross_repo: str = (f"This pull request came from a fork."
                           if pr["isCrossRepository"] else "")
        ...
        embed.add_field(
            name="Participants:",
            value="".join(participant_strings)
            if participant_strings else f"No participants",
            inline=True,
        )
        embed.add_field(
            name="Assignees:",
            value="".join(assignee_strings)
            if assignee_strings else f"No assignees",
            inline=True,
        )
        embed.add_field(
            name="Reviewers:",
            value="".join(reviewer_strings)
            if reviewer_strings else f"No reviewers",
            inline=True,
        )
        ...
        if pr["labels"]:
            embed.add_field(
                name=":label: Labels:",
                value=" ".join([f"`{lb}`" for lb in pr["labels"]]),
                inline=False,
            )

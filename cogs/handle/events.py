...
class Events(commands.Cog):
    ...
    async def build_guild_embed(self,
                                guild: discord.Guild,
                                state: bool = True) -> discord.Embed:
        if state:
        ...
        embed.add_field(name="Created at",
                        value=str(guild.created_at.strftime("%e, %b %Y")))
        embed.add_field(name="Channels",
                        value=str(len(guild.channels) - len(guild.categories)))
    ...
    async def on_message(self, message) -> None:
        ...
        if all([
                self.bot.user in message.mentions[:1],
                len(message.content) < 23, can_send
        ]):
            embed = discord.Embed(
                color=0xEFEFEF,
                title=None,

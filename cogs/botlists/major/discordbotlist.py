...
class DiscordBotListStats(commands.Cog):
    ...
    async def post_dblst_stats(self) -> None:
        async with self.bot.session.post(f'https://discordbotlist.com/api/v1/bots/{self.bot.user.id}/stats', json={'guilds': len(self.bot.guilds), 'users': int(sum(g.member_count for g in self.bot.guilds))}, headers={'Content-Type': 'application/json', 'Authorization': self.token}) as res:
            if res.status != 200:
                res = await res.json()

...
def dir_line_count(directory) -> int:
    return sum(
        map(lambda item: item_line_count(join(directory, item)), os.listdir(directory))
    )


LINES_OF_CODE: int = sum(
    [dir_line_count("./cogs"), dir_line_count("./ext"), dir_line_count("./core")]
)
...
class BotInfo(commands.Cog):
    ...
    @commands.command(
        name="uptime", aliases=["--uptime", "-uptime", "up", "--up", "-up"]
    )
    ...
    async def invite_command(self, ctx: commands.Context) -> None:
        ...
        embed.set_author(
            icon_url=self.bot.user.avatar_url, name="Invite me to your server!"
        )
        await ctx.send(embed=embed)
    ...
    async def stats_command(self, ctx: commands.Context) -> None:
        ...
        memory: str = "**{:.3f}GB** of RAM".format(process.memory_info()[0] / 2.0 ** 30)
